...
class CajaApiView(APIView):
    ...
    def get(self, request):     
        serializers = CajaSerializer(Caja.objects.filter(estado=True), many=True)
...
class CajaApiViewDetail(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            serializer = CajaSerializer(caja.objects.filter(estado=True), many=False)
            return Response(serializer.data)
        except Caja.DoesNotExist:
            return Response({'Error': 'Caja no encontrado'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def delete(self, request, pk):
        try:
            ...
            caja.estado = False
            caja.save()
            return Response({'message': 'Caja eliminado correctamente'}, status=status.HTTP_204_NO_CONTENT)
        except Caja.DoesNotExist:
            return Response({'Error': 'Caja no encontrado'}, status=status.HTTP_404_NOT_FOUND)
